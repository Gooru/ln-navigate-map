...
public class NavigationVerticle extends AbstractVerticle {
  ...
  public void start(Future<Void> startFuture) {
    ...
    eb.localConsumer(Constants.EventBus.MBEP_NAVIGATE, this::processMessage)
  ...
  private void processNextCommand(Message<JsonObject> message) {
    ...
    new ContextProcessor(vertx).fetchContext(message)
        .compose(navigateProcessorContext -> new PathFinderProcessor(vertx)
            .findNext(navigateProcessorContext))
        .compose(
            ar -> new ContentServer(vertx, future,
                new RemoteAssessmentCollectionFetcher(client, remoteUriLocator))
  ...
  private void triggerPostProcessing(PostProcessorPayloadCreator postProcessorPayloadCreator) {
    DeliveryOptions options =
        DeliveryOptionsBuilder
            .createDeliveryOptionsWithMsgOp(Constants.Message.MSG_OP_POSTPROCESS_NEXT);
    vertx.eventBus()
        .send(Constants.EventBus.MBEP_POST_PROCESS, postProcessorPayloadCreator.getResponse(),
            options);
  }

  private void persistNewContext(PostProcessorPayloadCreator postProcessorPayloadCreator,
      String user) {
    ...
    if (newContext != null) {
      ...
      vertx.eventBus().send(Constants.EventBus.MBEP_USER_CONTEXT, newContext,
          new DeliveryOptions()
              .addHeader(Constants.Message.MSG_OP, Constants.Message.MSG_OP_CONTEXT_SET)
