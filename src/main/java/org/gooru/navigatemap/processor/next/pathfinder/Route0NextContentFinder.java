  ...
  private ContentAddress findNextVerifiedContent() {
    ...
    if (route0ContentFinderContext.getCollectionId() != null) {
      if (route0ContentFinderContext.getClassId() == null) {
        models = getRoute0ContentFinderDao()
            .fetchNextContentsFromRoute0ForIL(route0ContentFinderContext);
      } else {
        models = getRoute0ContentFinderDao()
            .fetchNextContentsFromRoute0InClass(route0ContentFinderContext);
      }
    } else {
      if (route0ContentFinderContext.getClassId() == null) {
        models = getRoute0ContentFinderDao()
            .fetchAllContentsFromRoute0ForIL(route0ContentFinderContext);
      } else {
        models = getRoute0ContentFinderDao()
            .fetchAllContentsFromRoute0InClass(route0ContentFinderContext);
      }
    }
