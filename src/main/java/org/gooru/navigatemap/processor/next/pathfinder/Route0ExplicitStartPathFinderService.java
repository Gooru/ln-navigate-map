 ...
 * <p>
 * In case pathId is provided, it is used to find specified item. In case path id is not provided, it is assumed that
 * UL are provided and user is asking to start specified lesson. In this case, the first item on specified lesson is
 * returned.
 *
...
class Route0ExplicitStartPathFinderService implements PathFinder {
    ...
    private UserRoute0ContentDetailModel startSpecifiedCollection() {
        if (context.getClassId() != null) {
            return getRoute0ContentFinderDao().fetchSpecifiedItemFromRoute0InClass(route0ContentFinderContext);
        } else {
            return getRoute0ContentFinderDao().fetchSpecifiedItemFromRoute0ForIL(route0ContentFinderContext);
        }
    }

    private UserRoute0ContentDetailModel startSpecifiedLesson() {
        if (context.getClassId() != null) {
            return getRoute0ContentFinderDao().fetchFirstItemFromLessonForRoute0InClass(route0ContentFinderContext);
        } else {
            return getRoute0ContentFinderDao().fetchFirstItemFromLessonForRoute0ForIL(route0ContentFinderContext);
        }
    }
