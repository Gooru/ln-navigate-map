...
public interface UserCompetencyCompletionDao {
   ...
   */
  @SqlBatch(
      "insert into user_competency_status (user_id, comp_mcomp_id, completion_status) values(:userId::uuid,"
  ...
  void markCompetencyCompletedOrMastered(@Bind("userId") String userId,
      @Bind("competency") List<String> competency,
   ...
   */
  @SqlQuery(
      "select comp_mcomp_id from user_competency_status where comp_mcomp_id = any(:competencyList) "
  ...
  List<String> findCompetenciesBasedOnCompletionStatusForUserInGivenList(
