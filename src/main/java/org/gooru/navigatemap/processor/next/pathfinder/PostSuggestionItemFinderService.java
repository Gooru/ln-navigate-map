 ...
 * In this sense it is similar to {@link StraightPathFinderService}, however, this may extend in
 * future hence pulling it out for proper organization of code with respect to events in flow.
 *
 * @author ashish.
 */
class PostSuggestionItemFinderService implements PathFinder {
    ...
    public PathFinderResult findPath(PathFinderContext context) {
        return new PathFinderResult(ContentFinderFactory
            .buildAlternatePathUnawareMainPathContentFinder(dbi, ContentFinderCriteria.CRITERIA_NON_SKIPPABLE)
            .findContent(context));
    }
}
