...
public interface SuggestionCardDao {
  ...
  @SqlQuery(
      "select id, title, format, subformat, thumbnail, metadata, taxonomy from collection where id = any"
          + "(:collections)")
  ...
  @SqlQuery(
      "select collection_id, sum(case when content_format = 'resource' then 1 else 0 end) as resource_count, "
  ...
  List<CollectionRQCount> fetchRQCountForCollections(
      @Bind("collections") PGArray<UUID> collections);
}
