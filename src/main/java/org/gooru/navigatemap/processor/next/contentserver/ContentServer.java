...
public class ContentServer {
  ...
  public ContentServer(Vertx vertx, Future<JsonObject> future,
      RemoteAssessmentCollectionFetcher fetcher) {
    this.vertx = vertx;
  ...
  public void serveContent(NavigateProcessorContext npc) {
    ...
    if (npc.getCtxSuggestions().hasSuggestions()) {
      ...
      JsonObject result = ResponseBuilder
          .createSuccessResponseBuilder(navigateProcessorContext.responseContext(),
              new JsonObject())
          .buildResponse();
    ...
    if (AppConfiguration.getInstance().serveContentDetails()) {
      ...
      JsonObject result = ResponseBuilder
          .createSuccessResponseBuilder(navigateProcessorContext.responseContext(),
              new JsonObject()
                  .put("id", Objects
                      .toString(navigateProcessorContext.responseContext().getCurrentItemId(),
                          null))
                  .put("type", Objects
                      .toString(
                          navigateProcessorContext.responseContext().getCurrentItemType().getName(),
                          null))
                  .put("subtype",
                      Objects.toString(
                          navigateProcessorContext.responseContext().getCurrentItemSubtype(),
                          null)))
          .buildResponse();
      completionFuture.complete(result);
    }
  ...
  private JsonObject serveSuggestions() {
    ...
    return new SuccessResponseBuilder(navigateProcessorContext.responseContext(), suggestions)
        .buildResponse();
  }
