...
/**
 * This is the main workhorse to find the content on path. This class triggers different kind of
 * flows e.g. straight path flow, explicit start flow etc by calling and delegating different
 * services. Based on the outcome of service, this class decides what needs to be done with respect
 * to API response by modifying {@link NavigateProcessorContext}
 *
...
public class PathFinderProcessor {
  ...
  public Future<NavigateProcessorContext> findNext(
      NavigateProcessorContext navigateProcessorContext) {
  ...
  /*
   * Suggestion orientation implies that course must be navigator course. However, in navigator
   * courses' flow associated with class, there is a fork. This fork is based on whether
   * milestone oriented view is being played or the play is done based on classic CUL view
   */
  private void handleSuggestionsOrientedRoute() {
    initializeMilestoneViewApplicability();
    if (npc.needToStartCourse()) {
      LOGGER.debug("Need to start course");
      PathFinderResult result =
          PathFinderFactory.buildCourseStartPathFinderService()
      ...
      PathFinderResult result =
          PathFinderFactory
              .buildSuggestionsAwarePathFinderService(npc.requestContext().getRouteContextData())
              .findPath(PathFinderContext.buildContext(npc));
  ...
  private void initializeMilestoneViewApplicability() {
  ...
  private void serveTheSuggestions(PathFinderResult result) {
    switch (result.getSuggestedContentType()) {
      case Assessment:
        result.getSuggestions()
            .forEach(suggestedItem -> npc.getCtxSuggestions().addAssessment(suggestedItem));
        break;
      case Collection:
        result.getSuggestions()
            .forEach(suggestedItem -> npc.getCtxSuggestions().addCollection(suggestedItem));
        break;
      case Unit:
      case Course:
      case Lesson:
    ...
    npc.getCtxSuggestions()
        .setSuggestedContentSubType(result.getSuggestedContentSubType().getName());
  ...
  private void handleNoSuggestionsRoute() {
    if (npc.userExplicitlyAskedToStartCollection()) {
      ...
      PathFinderResult result =
          PathFinderFactory.buildSpecifiedItemFinderService()
              .findPath(PathFinderContext.buildContext(npc));
      serveTheContent(result.getContentAddress());
    } else {
      LOGGER.debug("Will find next item on no suggestions route.");
      PathFinderResult result =
          PathFinderFactory.buildStraightPathFinderService()
              .findPath(PathFinderContext.buildContext(npc));
      serveTheContent(result.getContentAddress());
    }
  }
