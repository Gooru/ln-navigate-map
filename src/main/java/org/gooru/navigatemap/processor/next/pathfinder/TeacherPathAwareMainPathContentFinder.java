...
/**
 * This class encapsulates the logic to find next content on main path which is defined by Criteria.
 * If such content exists, then if there is a teacher path associated with that content, teacher
 * path is returned. if there is no teacher path, then content itself is returned. If there is no
 * content, no content address is populated in result.
 *
  ...
  public ContentAddress findContent(PathFinderContext context) {
    ContentAddress result =
        ContentFinderFactory
            .buildAlternatePathUnawareMainPathContentFinder(dbi, contentFinderCriteria)
            .findContent(context);
