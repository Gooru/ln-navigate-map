...
class TeacherSuggestionAddedNotificationCoordinator implements NotificationCoordinator {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(TeacherSuggestionAddedNotificationCoordinator.class);
    ...
    public void coordinateNotification() {
        ...
        if (userPathMap.size() > command.getUserIds().size()) {
            LOGGER.warn("Size of user path map is greater than that of users specified. Won't send notifications");
            return;
        }
        sendNotifications();
    }
    ...
    private void sendNotifications() {
        Producer<String, String> producer = KafkaProducerRegistry.getInstance().getTeacherSuggestionKafkaProducer();
        NotificationSenderService senderService =
            NotificationSenderService.build(producer, AppConfiguration.getInstance().getNotificationTopic());
