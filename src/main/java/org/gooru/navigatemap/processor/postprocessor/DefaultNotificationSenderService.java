...
class DefaultNotificationSenderService implements NotificationSenderService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultNotificationSenderService.class);
        ...
        try {
            ...
            if (producer != null) {
                producer.send(producerRecord, (metadata, exception) -> {
                    if (exception == null) {
                        LOGGER.debug("Message Delivered, got metadata: Offset : " + metadata.offset() + " : Topic : " +
