...
interface AddSystemSuggestionDao {
  ...
  @GetGeneratedKeys
  @SqlUpdate(
      "insert into user_navigation_paths (ctx_user_id, ctx_course_id, ctx_unit_id, ctx_lesson_id, "
  ...
  @SqlQuery(
      "select id from user_navigation_paths where ctx_course_id = :ctxCourseId and ctx_unit_id = "
  ...
  @SqlQuery(
      "select id, ctx_user_id, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_class_id, ctx_collection_id, "
          ...
          + ":suggestionType;")
  AlternatePath findUserPathForRoute0WithClass(
      @BindBean AddSystemSuggestionCommand.AddSystemSuggestionsBean bean);
  ...
  @SqlQuery(
      "select id, ctx_user_id, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_class_id, ctx_collection_id, "
          ...
          + ":suggestionType;")
  AlternatePath findUserPathForRoute0WithoutClass(
      @BindBean AddSystemSuggestionCommand.AddSystemSuggestionsBean bean);

  @SqlQuery(
      "select exists (select 1 from collection l, class c where l.course_id = :ctxCourseId and l.unit_id = "
  ...
  @SqlQuery(
      "select exists (select 1 from collection l where l.course_id = :ctxCourseId and l.unit_id = :ctxUnitId "
  ...
  Boolean validateContextInformationForRoute0WithoutClass(
      @BindBean AddSystemSuggestionCommand.AddSystemSuggestionsBean bean);

  @SqlQuery(
      "select exists (select 1 from course l, class c where l.id = :ctxCourseId and l.is_deleted = false and "
          + "c.id = :ctxClassId and c.course_id = l.course_id and c.is_deleted = false)")
  Boolean validateContextInformationForRoute0WithClass(
      @BindBean AddSystemSuggestionCommand.AddSystemSuggestionsBean bean);
}
