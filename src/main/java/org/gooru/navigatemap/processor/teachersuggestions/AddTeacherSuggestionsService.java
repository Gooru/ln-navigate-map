...
class AddTeacherSuggestionsService {
  ...
  private Map<String, Integer> transformResult(int[] input,
      List<UUID> usersNotHavingSpecifiedSuggestions) {
    ...
    if (input.length != usersNotHavingSpecifiedSuggestions.size()) {
      LOGGER.warn(
          "Size of users list does not match returned id. May be race condition. Won't return anything");
      return Collections.emptyMap();
    }
