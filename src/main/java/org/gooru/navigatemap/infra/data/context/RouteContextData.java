...
/**
 * This class encapsulates the data object in context for Next API. The context data key would be used for state
 * which is pass through for caller but is needed on the backend. The data state will evolve and this class will
 * provide a type safe method to dereference the object.
 * In a nutshell encoded string will denote a JSON object which is converted to string and then encoded using base64.
 *
