...
import org.gooru.navigatemap.infra.data.NavigateMessageContext;
...
public class ContextProcessor {
      ...
      if (context.needsLastState() && !messageContext.isUserAnonymous()) {
        vertx.eventBus().<JsonObject>send(Constants.EventBus.MBEP_USER_CONTEXT, new JsonObject(),
            DeliveryOptionsBuilder
                .createDeliveryOptionsForFetchUserContext(timeout, getUserContextKey()),
            asyncResult -> {
  ...
  private String getUserContextKey() {
    return ContextUtil
        .createUserContextKey(messageContext.getUserId(), context.getCourseId(),
            context.getClassId());
  }
